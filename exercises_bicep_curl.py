...
def run(mode="free", target_reps=None, target_time=None):
    ...
    window_name = "Bicep Curl Exercise"

    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:

        # ─── CALIBRATION PHASE ───────────────────────────────────────────────
        print("Calibration: Please stand straight and fully extend your arms (elbows straight).")
        calib_angles = []
        ...
        while cap.isOpened() and (time.time() - calib_start) < CALIB_DURATION:
            ...
            if results.pose_landmarks:
                ...
                left_vis = (lm[11].visibility + lm[13].visibility + lm[15].visibility) / 3
                right_vis = (lm[12].visibility + lm[14].visibility + lm[16].visibility) / 3
            ...
            cv2.putText(frame, "CALIBRATION - Fully extend your arms!", (50, 80),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.3, (0, 255, 255), 3)
            cv2.putText(frame, f"Current elbow angle: {int(angle)}°", (50, 130),
        ...
        rest_angle = np.mean(calib_angles) if calib_angles else 170.0
        print(f"→ Rest angle (full arm extension) detected: {rest_angle:.1f}°")
        ...
        while cap.isOpened() and not completed:
            ...
            if results.pose_landmarks:
                ...
                key_points = [
                    lm[mp_pose.PoseLandmark.LEFT_SHOULDER], lm[mp_pose.PoseLandmark.LEFT_ELBOW], lm[mp_pose.PoseLandmark.LEFT_WRIST],
                    lm[mp_pose.PoseLandmark.RIGHT_SHOULDER], lm[mp_pose.PoseLandmark.RIGHT_ELBOW], lm[mp_pose.PoseLandmark.RIGHT_WRIST]
                ]
                ...
                if avg_visibility > 0.6:
                    ...
                    left_visibility = (lm[11].visibility + lm[13].visibility + lm[15].visibility) / 3
                    right_visibility = (lm[12].visibility + lm[14].visibility + lm[16].visibility) / 3
                        ...
                        if angle > 165:
                            if stage == "up":
                                ...
                                if down_angle is not None and up_angle is not None:
                                    ...
                                    if down_angle > 168 and up_angle < 40:
                                        good_reps += 1
                                    else:
                                        wrong_reps += 1
                                down_angle = None
            ...
            # DISPLAY
            if mode == "reps" and target_reps is not None:
            ...
            if mode == "time" and target_time is not None:
            ...
            if mode == "reps" and target_reps is not None and reps >= target_reps:
            ...
            if mode == "time" and target_time is not None and (time.time() - start_time) >= target_time:
